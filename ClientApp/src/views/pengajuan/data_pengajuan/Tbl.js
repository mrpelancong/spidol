...
export class Tbl extends React.Component {
    constructor(props) {
        super(props);
        let vendorAcess = this.props.vendor ? this.props.vendor : false ;
        this.state = {
            request : this.props.request, expired : this.props.expired, active : this.props.active, vendor : vendorAcess, 
            ...
            const listPengajuan = props.map((data, index) => {
                      //cek file pengajuan
                        ...
                        if (data.status_pengajuan == 0) {
                            statusPengajuan = 'Pengisian Berkas';
                            statusID = (
                          ...
                          if (data.proses) {
                            if (data.idCard === null) {
                              ...
                              statusID = (
                                ...
                                <button className="btn btn-sm btn-success btn-block"> Cetak ID Card</button>
                                </>
                              )
                          ...
                          if (data.idCard === null) {
                            ...
                            statusID = (
                              <button className="btn btn-primary btn-sm btn-block" onClick={() => this.setState({
                                modalNomorPegawai : true,
                                data_pegawai : {
                                  pengajuanId : data.id,
                                  nikPegaawai : data.nik,
                        ...
                        if (this.state.request) {
                          if (data.status_pengajuan == 2) { 
                            if(data.idCard === null && this.state.expired === false){
                              return(
                                <tr key={data.id}>
                            ...
                            if (data.idCard) {
                              if (this.state.expired && data.idCard.expired) {
                                return(
                                  <tr key={data.id}>
                                    <td>{data.nik} <br></br> {data.tgl_pengajuan}</td>
                                    <td>{data.vendor.nama_vendor}</td>
                              ...
                              if (this.state.active && data.idCard.expired === false) {
                                return(
                                  <tr key={data.id}>
                                    <td>{data.nik} <br></br> {data.tgl_pengajuan}</td>
                                    <td>{data.vendor.nama_vendor}</td>
