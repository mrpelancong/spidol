...
export class Tbl extends React.Component {
    ...
    TBody = (props) => {
        if (props) {
            let dataAkses = props.map((data, index) => {
                if (data.akses_name.toLowerCase() != 'pegawai' && data.akses_name.toLowerCase() != 'pimpinan pertamina') {
                    return (
                        <tr key={data.id}>
                            <td>{index + 1}</td>
                            <td>{data.akses_name}</td>
                            <td>
                                <center>
    ...
    loadData = (id_data = "") => {
        this.props.getData(0, id_data).then((result) => {
            if (id_data === "") {
                this.setState({
            ...
            <>
                ...
                >
                    ...
                    <CForm onSubmit={this.handleSubmit}>
                        <CModalBody>
                            <CFormGroup>
                                <CLabel htmlFor="akses_name">Akses Name</CLabel>
                                <CInput id="akses_name" type="text" name="akses_name" placeholder="Akses Name" />
                            </CFormGroup>
                        </CModalBody>
                        <CModalFooter>
                            <CButton
